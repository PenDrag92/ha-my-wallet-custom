...
export function periodMetrics(points, { position = false, accountingChanged = false } = {}) {
  ...
  const result = {
    ...
    capital: null, dividends: null, gain: null, return: null, reason: null,
  };
  ...
  for (let index = 0; index < points.length; index++) {
    ...
    if (!prior) continue;
    if (point.revision && prior.revision && point.revision !== prior.revision) result.reason = "accounting";
    ...
    if (capital < -0.005 || dividends < -0.005) result.reason = "accounting";
    if (position && finite(point.units) && finite(prior.units) && Math.abs(point.units - prior.units) > 1e-8 && Math.abs(capital) < 0.005) result.reason = "accounting";
